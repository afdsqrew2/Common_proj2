...
public class WinDialog : MonoBehaviour
{
		...
		public void Hide ()
		{
				...
				firstStarFading.transform.Find ("Effect").GetOrAddComponent<ParticleEmitter>().emit = false;
				secondStarFading.SetBool ("Running", false);
				secondStarFading.transform.Find ("Effect").GetOrAddComponent<ParticleEmitter> ().emit = false;
				thirdStarFading.SetBool ("Running", false);
				thirdStarFading.transform.Find ("Effect").GetOrAddComponent<ParticleEmitter> ().emit = false;
		}
		...
		private IEnumerator ShowEffectCouroutine (Transform fadingStar)
		{
				...
				fadingStar.Find ("Effect").GetOrAddComponent<ParticleEmitter> ().emit = true;
		}
