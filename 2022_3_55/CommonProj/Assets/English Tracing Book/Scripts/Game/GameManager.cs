...
public class GameManager : MonoBehaviour
{
	...
	void Awake ()
	{
		completeEffect = transform.GetOrAddComponent<ParticleEmitter>();
	...
	void Update ()
	{
		...
		if (Input.GetMouseButtonDown (0)) {
			if (!shape.completed)
				brightEffect.GetOrAddComponent<ParticleEmitter> ().emit = true;
			...
			brightEffect.GetOrAddComponent<ParticleEmitter> ().emit = false;
			DisableHand ();
